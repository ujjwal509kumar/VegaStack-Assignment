...
async function listUsersHandler(req: AuthenticatedRequest) {
  try {
    ...
    // Both admins and regular users respect privacy settings in discover
    // Admins can see all users only in /api/admin/users
    const { users, total } = await db.admin.listUsers(page, limit);
    
    // Filter users based on visibility (applies to both admins and regular users)
